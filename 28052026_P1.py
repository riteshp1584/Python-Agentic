...
def supervisor_node(state: TeamState):
    ...
    # Count how many times the validator has actually appended an AIMessage to the list
    # The first query is a HumanMessage. Math tool returns a ToolMessage.
    # General LLM conversational responses are AIMessages.
    ai_message_count = sum(1 for m in state["messages"] if m.__class__.__name__ == "AIMessage")
    tool_message_count = sum(1 for m in state["messages"] if m.__class__.__name__ == "ToolMessage")

    prompt = (
        "You are the Portfolio Supervisor Manager. Look at the history logs.\n"
        f"- Math Tool Executions Present: {tool_message_count}\n"
        f"- Analyst Review Messages Present: {ai_message_count}\n\n"
        "Your strict sequential rules are:\n"
        "1. If the Math Tool Execution count is 0, you MUST return 'math_worker'.\n"
        "2. If the Math Tool Execution count is 1, but the Analyst Review Message count is less than 2, you MUST return 'validator_worker'.\n"
        "3. If BOTH tasks have been completed sequentially, you MUST return 'FINISH'."
    )
...
def validator_worker_node(state: TeamState):
    ...
    prompt = (
        "Look at the actual tool return value in the history. Provide a short "
    ...
    # We commit it directly as a clean AIMessage
